...
class CHANGE extends Component {
    ...
    convertCurrency = () => {
        fetch('https://currencyflask.herokuapp.com/api/exchange?from='+this.state.from+'&to='+this.state.to+'&much='+this.state.much)
        .then(res => res.json())
        .then((data) => {
            this.setState({ result: data.getback.toFixed(5)})
        })
        .catch(console.log)
    }

    handleChange = (event) => {
      this.setState({much: event.target.value});
    };
...
export default withRouter(withStyles(styles)(CHANGE));